...
try {
	...
	$sth = $sql->query("
		...
		P1.summoner_name AS top, P1.ppl_id AS p1id,
		P2.summoner_name AS mid, P2.ppl_id AS p2id,
		P3.summoner_name AS jng, P3.ppl_id AS p3id,
	   	P4.summoner_name AS adc, P4.ppl_id AS p4id,
		P5.summoner_name AS sup, P5.ppl_id AS p5id,
		P6.summoner_name AS sub1, P6.ppl_id AS p6id,
		P7.summoner_name AS sub2, P7.ppl_id AS p7id
	   	FROM teams AS t
	...
	/*  people selection in here */
		displayPeople($summoners, "top", $current->p1id, $current->top);
		displayPeople($summoners, "mid", $current->p2id, $current->mid);
		displayPeople($summoners, "jng", $current->p3id, $current->jng);
		displayPeople($summoners, "adc", $current->p4id, $current->adc);
		displayPeople($summoners, "sup", $current->p5id, $current->sup);
		displayPeople($summoners, "sub1", $current->p6id, $current->sub1);
		displayPeople($summoners, "sub2", $current->p7id, $current->sub2);
	echo "</tr>";
	echo "</table>";
} catch(PDOException $e) {
	echo $e->getMessage();
